...
In `video_translator_gui.py`, replace the block that starts with `# ── GUI colours — Neon Dark theme` (or whatever the em-dash cleanup renamed it to) up to and including `_MONO_LG = ...` with:
