...
def _subs_only_runtime(tmp_dir):
    """Minimal runtime for the subs-only path (no TTS, no mux)."""
...
class PipelineRunnerTests(unittest.TestCase):
    ...
    def test_default_output_goes_to_the_configured_output_dir(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            video_path = os.path.join(tmp_dir, "input.mp4")
            Path(video_path).write_bytes(b"fake")
            out_dir = os.path.join(tmp_dir, "chosen")
            ...
            result = translate_video(
                ...
                subs_only=True,
                segments_override=[
                    {"start": 0.0, "end": 1.0, "text_src": "h", "text_tgt": "c"}
                ],
                difficulty_profile_enabled=False,
                runtime=_subs_only_runtime(tmp_dir),
            )

            self.assertEqual(result["srt"], os.path.join(out_dir, "input_it.srt"))
            self.assertTrue(os.path.isdir(out_dir))

    def test_default_output_falls_back_to_the_app_folder(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            video_path = os.path.join(tmp_dir, "input.mp4")
            Path(video_path).write_bytes(b"fake")
            ...
            expected_dir = os.path.join(tmp_dir, "VideoTranslatorAI")
            self.assertEqual(result["srt"],
                             os.path.join(expected_dir, "input_it.srt"))
